...
class Site
{
    ...
    private function __ensureDirs()
    {
        ...
        try {
            ...
            throw new LogicException('Could not ensure that the directory ' . $work . '/tmp is present and writable.');
        }
    ...
    public function loadMetadata()
    {
        if (!$this->metadata) {
            if ($this->draftExists()) {
                $this->metadata = @unserialize($this->fs->read($this->draftMarkerPath()));
     ...
     * @return bool
     * @throws \Exception
     * @internal param string $path protected function findMenus(Page $page, $path)protected function findMenus(Page
     *     $page, $path)
     * {
     * $menu = [];
     *
    ...
    protected function findMenus(Page $page, $path)
    {
        foreach ($page->query('[class*="' . Menu::SC_MENU_BASE_CLASS . '"]') as $menu) {
            ...
            if (array_search($path, $this->metadata['menus'][$name]['pages']) === false) {
    ...
    {
        $base = $this->base();
        if (!empty($this->config['pages.use_document_relative_paths']) && strpos($path, $base) !== 0) {
    ...
    protected function base()
    {
        ...
        if (strpos($self, '/' . $this->config['SERVICE_URL']) === 0) {
     ...
     */
    protected function updateMenuMetadata()
    {
        // Set menu order and menu item text for all page files in metadata
        if (isset($this->metadata['menus']['main']) &&
            ($menuItems = $this->metadata['menus']['main']['items']) &&
            !empty($this->metadata['pages'])
        ) {
            foreach ($this->metadata['pages'] as $path => &$properties) {
    ...
    public function updatePageFiles($pages, $pagesMetadata)
    {
        ...
        // Update menus
        $this->updateMenus($pagesMetadata);
        // Save metadata
        ...
        foreach ($this->metadata['menus']['main']['pages'] as $path) {
            ...
            foreach ($page->query('[class*="' . Menu::SC_MENU_BASE_CLASS . '"]') as $menu) {
                ...
                $page->findAndReplace(
                    ...
                    $menu->render($this->config['pages.nav.item_template'], function ($url) use ($path) {
